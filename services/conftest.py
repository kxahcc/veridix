...
def _safe_socketpair(*args, **kwargs):
    """Windows-friendly socketpair with bounded waits and retries.

    Python's Windows fallback for ``socket.socketpair`` can block forever in
    ``accept()`` under load. This replacement keeps loopback socket creation,
    but bounds the accept with a timeout and retries before failing.
    """
