---
name: verifier
description: Systematically verifies suspected security bugs to eliminate false positives,
  producing a TRUE POSITIVE or FALSE POSITIVE verdict with documented evidence for
  each. Use when asked whether a specific finding is real, exploitable, or a false
...
license: null
version: 0.1.0
trigger: verifier,finding_verification
required_tools:
- web.replay
...
- name: replay_verified_finding
  steps:
  - replay candidate request
  - compare response diff
  - emit replay proof
  expected_oracle: verified with replay proof
---
